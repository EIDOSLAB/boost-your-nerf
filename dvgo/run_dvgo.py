...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs, cfg,
    ...
    if len(psnrs):
        mean_psnrs, mean_ssims, mean_lpips_vgg, mean_lpips_alex = 0, 0, 0, 0
        mean_psnrs = np.mean(psnrs)
        print('Testing psnr', mean_psnrs, '(avg)')
        if eval_ssim: 
            mean_ssims = np.mean(ssims)
            print('Testing ssim', mean_ssims, '(avg)')
        if eval_lpips_vgg: 
    ...
    if savedir is not None and dump_images:
        for i in trange(len(rgbs)):
            rgb8 = rgbs[i]
            if rgb8.shape[-1] == 1:
                rgb8 = np.concatenate((rgb8, np.zeros((800,800,2))), axis=2)
            rgb8 = utils.to8b(rgb8)
    ...
    return mean_psnrs, mean_ssims, mean_lpips_vgg, mean_lpips_alex
...
def create_new_model(cfg, cfg_model, cfg_train, xyz_min, xyz_max, stage, coarse_ckpt_path):
    ...
    resolutions = model_kwargs.pop('resolutions', [])
    if len(cfg_train.pg_scale):
        num_voxels = int(num_voxels / (2**len(cfg_train.pg_scale)))
        for i in range(len(resolutions)):
            resolutions[i] = int(resolutions[i] / (2**len(cfg_train.pg_scale)))
    if cfg.data.ndc:
...
def load_existed_model(args, cfg, cfg_train, reload_ckpt_path, model_class=None):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None, gate_type=None, pretrained_path=None):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
...
def train(args, cfg, data_dict, pretrained_path=None):
    # init
    print('train: start')
    eps_time = time.time()
